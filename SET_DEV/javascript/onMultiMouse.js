...
//>> -----
        ...
        function ion_multiMouseEvt( jUseType, jState, evt ){
            var jListenerList = ion_evtHandle( jUseType, jState, evt );
            for(var p = 0, jItem; jItem = jListenerList[ p++ ] ; ){
                if( !jItem.target.dispatchEvent( jItem.evt ) || jState === 'end' ){
                    var isPrevent = ( jState !== 'end' )? true : false;
                    ion_evtDefault.bind( isPrevent, jItem.id, evt );
                }
                // document.elementFromPoint(40,220)
            }
        }
        ...
        function ion_evtHandle( jUseType, jState, evt ){
            ...
            for(var p = 0, jItem; jItem = jEvtInfList[ p++ ] ; )
        ...
        var ion_evtDefault = {
                bind: function( isPrevent, jID, evt ){
                    var jPreventList = this.preventList,
                    ...
                    if( isPrevent ){
                        ...
                        var jIndex_prevent = jPreventList.indexOf( jID );
                        if( jIndex_prevent !== -1 ){
        ...
        function ion_getEvtInfList( jUseType, jState, evt ){
            ...
            for(var p = 0, jItem_evt; jItem_evt = jEvtList[ p++ ] ; ){
                if( isTouch )
        ...
        function ion_setAnnexTouchInf( jAnnex, evt ){
            jAnnex.touchIDList = [];
            for(var p = 0, jTouches = evt.touches, jItem; jItem = jTouches[ p++ ] ; )
        ...
        function ion_getDispatchInf( jEvtInf ){
            var jNewEvt = new CustomEvent( 'multimouse', {
                    bubbles: true,
                    cancelable: true,
                    detail: ion_getDetail( jEvtInf ),
                } );

            for(var p = 0, jItem; jItem = ArrEvtProp[ p++ ] ; )
